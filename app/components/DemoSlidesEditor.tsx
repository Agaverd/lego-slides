...
import { ChangeEvent, CSSProperties, DragEvent as ReactDragEvent, Fragment, PointerEvent as ReactPointerEvent, useCallback, useEffect, useRef, useState } from "react";
...
function Mockup({ block }: { block: Block }) {
  ...
  return <div className={`mockup-device-stage ${backgroundMode === "None" ? "is-transparent" : ""}`} style={stageStyle}>
    <div className={`device-composite ${model === "MacBook Air" ? "is-laptop" : "is-phone"}`} style={{ transform: `translate(${Number(c.horizontal ?? 0)}px, ${Number(c.vertical ?? 0)}px) scale(${Number(c.scale ?? 90) / 100})` }}>
      ...
      <div className="device-screen" style={screenStyle}>{src ? <img src={src} alt="Скриншот продукта" style={{ objectFit: "cover" }} /> : <div className="product-skeleton"><i /><i /><i /><b /></div>}</div>
...
function SlideCanvas({ slide, settings, selectedId, selectedIds = [], editingId, preview, showGrid = false, externalDragType, onSelect, onToggleSelect, onSelectMany, onEdit, onMoveResize, onResolveMove, onDuplicateAt, onContent, onDropBlock }: {
  slide: Slide; settings: PresentationSettings; selectedId: string | null; selectedIds?: string[]; editingId: string | null; preview: boolean; showGrid?: boolean;
  externalDragType?: BlockType | null; onSelect: (id: string | null) => void; onToggleSelect?: (id: string) => void; onSelectMany?: (ids: string[]) => void; onEdit: (id: string | null) => void; onMoveResize: (id: string, area: GridArea) => void; onResolveMove?: (id: string, area: GridArea, resolved: Record<string, GridArea>) => void; onDuplicateAt?: (id: string, area: GridArea) => void; onContent: (id: string, content: Record<string, unknown>) => void; onDropBlock?: (type: BlockType, origin: { x: number; y: number }) => void;
}) {
  ...
  const startPointer = (e: ReactPointerEvent, block: Block, mode: "move" | ResizeEdge) => {
    if (preview) return;
    if (editingId === block.id) { e.stopPropagation(); return; }
    e.stopPropagation(); e.preventDefault();
    if (e.shiftKey && mode === "move") { onToggleSelect?.(block.id); return; }
    if (!selectedIds.includes(block.id)) onSelect(block.id);
  ...
  const startMarquee = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (preview || event.button !== 0 || (event.target as HTMLElement).closest(".slide-block")) return;
    event.preventDefault(); const canvas = canvasRef.current; if (!canvas) return; const rect = canvas.getBoundingClientRect(); const start = { x: (event.clientX - rect.left) / rect.width * 1200, y: (event.clientY - rect.top) / rect.height * 675 }; const additive = event.shiftKey; let live = { x: start.x, y: start.y, w: 0, h: 0 };
    const move = (nextEvent: PointerEvent) => { const x = (nextEvent.clientX - rect.left) / rect.width * 1200; const y = (nextEvent.clientY - rect.top) / rect.height * 675; live = { x: Math.min(start.x, x), y: Math.min(start.y, y), w: Math.abs(x - start.x), h: Math.abs(y - start.y) }; setMarquee(live); };
    const up = () => { window.removeEventListener("pointermove", move); window.removeEventListener("pointerup", up); setMarquee(null); if (live.w < 3 && live.h < 3) { if (!additive) onSelect(null); return; } const metrics = getGridMetrics(settings); const hits = slide.blocks.filter((block) => { const box = { x: metrics.left + block.grid.x * (metrics.cellW + metrics.gap), y: metrics.top + block.grid.y * (metrics.cellH + metrics.gap), w: block.grid.w * metrics.cellW + (block.grid.w - 1) * metrics.gap, h: block.grid.h * metrics.cellH + (block.grid.h - 1) * metrics.gap }; return live.x < box.x + box.w && live.x + live.w > box.x && live.y < box.y + box.h && live.y + live.h > box.y; }).map((block) => block.id); onSelectMany?.(additive ? [...new Set([...selectedIds, ...hits])] : hits); };
    window.addEventListener("pointermove", move); window.addEventListener("pointerup", up);
  };
  ...
  return <div ref={canvasRef} className={`slide-canvas ${preview ? "is-preview" : ""}`} style={canvasStyle} onPointerDown={startMarquee} onDragOver={(e) => { if (onDropBlock) { e.preventDefault(); if (externalDragType) setGuide({ area: { ...dragOrigin(e), w: 2, h: 2 }, mode: "move" }); } }} onDragLeave={(e) => { if (!e.currentTarget.contains(e.relatedTarget as Node)) setGuide(null); }} onDrop={dropBlock} role="presentation">
...
export function DemoSlidesEditor() {
  ...
  const [currentId, setCurrentId] = useState(""); const [selectedIds, setSelectedIds] = useState<string[]>([]); const selectedId = selectedIds.at(-1) ?? null; const setSelectedId = useCallback((id: string | null) => setSelectedIds(id ? [id] : []), []); const [editingId, setEditingId] = useState<string | null>(null);
  ...
  const removeSelected = useCallback(() => { if (!selectedIds.length) return; const ids = new Set(selectedIds); updateCurrent((s) => ({ ...s, blocks: s.blocks.filter((b) => !ids.has(b.id)) })); setSelectedId(null); }, [selectedIds, updateCurrent, setSelectedId]);
  const toggleSelected = useCallback((id: string) => { setEditingId(null); setSelectedIds((ids) => ids.includes(id) ? ids.filter((item) => item !== id) : [...ids, id]); }, []);
  const selectMany = useCallback((ids: string[]) => { setEditingId(null); setSelectedIds(ids); }, []);
  const duplicateBlock = useCallback(() => { if (!selected || !current) return; const { columns, rows } = project.presentationSettings.grid; const area = findBestSpace(current.blocks, selected.grid.w, selected.grid.h, columns, rows, { x: selected.grid.x + 1, y: selected.grid.y + 1 }); if (!area) { setNotice("На слайде нет места для копии"); return; } const copy = { ...clone(selected), id: crypto.randomUUID(), grid: area }; updateCurrent((s) => ({ ...s, blocks: [...s.blocks, copy] })); setSelectedId(copy.id); setNotice(area.w < selected.grid.w || area.h < selected.grid.h ? "Копия уменьшена до свободного места" : "Блок продублирован"); }, [selected, current, project.presentationSettings.grid, updateCurrent, setSelectedId]);
  const duplicateBlockAt = useCallback((id: string, area: GridArea) => { const copyId = crypto.randomUUID(); updateCurrent((s) => { const source = s.blocks.find((block) => block.id === id); if (!source) return s; return { ...s, blocks: [...s.blocks, { ...clone(source), id: copyId, grid: area }] }; }); setSelectedId(copyId); setNotice("Копия размещена на холсте"); }, [updateCurrent, setSelectedId]);
  ...
  useEffect(() => {
    const key = (e: KeyboardEvent) => {
      ...
      if (input) return;
      if ((e.key === "Delete" || e.key === "Backspace") && selectedIds.length) { e.preventDefault(); removeSelected(); }
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "z") { e.preventDefault(); if (e.shiftKey) redo(); else undo(); }
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "y") { e.preventDefault(); redo(); }
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === "d") { e.preventDefault(); duplicateBlock(); }
    };
    window.addEventListener("keydown", key); return () => window.removeEventListener("keydown", key);
  }, [selectedIds.length, removeSelected, duplicateBlock, undo, redo, setSelectedId]);
  useEffect(() => {
  ...
  const addBlankSlideAfter = (afterId: string) => { const id = crypto.randomUUID(); const next: Slide = { id, order: 0, title: "Blank", background: "#FFFFFF", backgroundMode: "Image", backgroundStyle: "Solid", backgroundPreset: "wb-blue", blocks: [] }; commit((p) => { const insertionIndex = Math.max(0, p.slides.findIndex((slide) => slide.id === afterId) + 1); const slides = [...p.slides]; slides.splice(insertionIndex, 0, next); return { ...p, slides: slides.map((slide, index) => ({ ...slide, order: index })) }; }); setCurrentId(id); setSelectedId(null); setPresetOpen(false); };
  ...
  return <ThemeProvider theme="dark"><main className="editor-shell">
    ...
    <div className="workspace">
      <aside className="slides-panel"><div className="panel-title"><span>Слайды</span><small>{project.slides.length}</small></div><div className="slide-list">{project.slides.map((s, i) => <Fragment key={s.id}><div className={`thumbnail-row ${s.id === current.id ? "active" : ""}`} draggable tabIndex={0} role="button" onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { setCurrentId(s.id); setSelectedId(null); } }} onDragStart={() => setDragSlideId(s.id)} onDragOver={(e) => e.preventDefault()} onDrop={() => reorder(s.id)} onClick={() => { setCurrentId(s.id); setSelectedId(null); }}><span className="slide-number">{String(i + 1).padStart(2, "0")}</span><SlideThumbnail slide={s} settings={project.presentationSettings} /><div className="thumb-actions"><Button view="flat" onClick={(e) => { e.stopPropagation(); duplicateSlide(s.id); }} title="Дублировать"><Icon data={Copy} size={14} /></Button><Button view="flat-danger" onClick={(e) => { e.stopPropagation(); deleteSlide(s.id); }} title="Удалить"><Icon data={TrashBin} size={14} /></Button></div></div>{i === project.slides.length - 1 && <button className="contextual-add-slide" type="button" aria-label="Добавить пустой слайд в конец" title="Добавить пустой слайд" onClick={() => addBlankSlideAfter(s.id)}><span aria-hidden="true">{String(i + 2).padStart(2, "0")}</span><i><Icon data={Plus} size={22} /></i></button>}</Fragment>)}</div><div className="add-wrap"><Button className="add-slide-button" view="action" width="max" onClick={() => setPresetOpen(!presetOpen)}><Icon data={Plus} size={16} />Добавить слайд</Button>{presetOpen && <div className="popover presets"><strong>Выберите раскладку</strong>{Object.keys(presets).map((p) => <button key={p} onClick={() => addSlide(p as keyof typeof presets)}><span className="preset-icon" />{p}</button>)}</div>}</div></aside>
      <section className="canvas-area"><div className="canvas-toolbar"><span>Слайд {current.order + 1}</span><span>16:9 · {project.presentationSettings.grid.columns} × {project.presentationSettings.grid.rows}</span></div><Button className="viewfinder-preview-toggle" view="flat" size="l" selected={gridVisible} aria-label={gridVisible ? "Скрыть сетку" : "Показать сетку"} title={gridVisible ? "Скрыть сетку" : "Показать сетку"} onClick={() => setGridVisible((visible) => !visible)}><Icon data={gridVisible ? Eye : EyeSlash} size={22} /></Button><div className="canvas-stage"><SlideCanvas slide={current} settings={project.presentationSettings} selectedId={selectedId} selectedIds={selectedIds} editingId={editingId} preview={false} showGrid={gridVisible} externalDragType={dockDragType} onSelect={(id) => { setSelectedId(id); if (!id) setEditingId(null); }} onToggleSelect={toggleSelected} onSelectMany={selectMany} onEdit={setEditingId} onMoveResize={(id, area) => updateCurrent((s) => ({ ...s, blocks: s.blocks.map((b) => b.id === id ? { ...b, grid: area } : b) }))} onResolveMove={(id, area, resolved) => updateCurrent((s) => ({ ...s, blocks: s.blocks.map((b) => b.id === id ? { ...b, grid: area } : resolved[b.id] ? { ...b, grid: resolved[b.id] } : b) }))} onDuplicateAt={duplicateBlockAt} onContent={(id, content) => updateCurrent((s) => ({ ...s, blocks: s.blocks.map((b) => b.id === id ? { ...b, content } : b) }))} onDropBlock={addBlock} /></div><div className="canvas-actions"><BlockDock onAdd={addBlock} onDragType={setDockDragType} /></div></section>
      <aside className="inspector"><div className="context-title"><span>{selectedIds.length > 1 ? `Выбрано: ${selectedIds.length}` : selected ? blockLabels[selected.type].label : "Страница"}</span>{selected && <small>{selectedIds.length > 1 ? "group" : selected.type}</small>}</div>{selected ? <BlockInspector block={selected} patch={patchSelectedContent} upload={upload} duplicate={duplicateBlock} remove={removeSelected} multiple={selectedIds.length > 1} /> : <PageInspector slide={current} slides={project.slides} settings={project.presentationSettings} updateSlide={(patch) => updateCurrent((slide) => ({ ...slide, ...patch }))} updateSettings={updatePresentationSettings} />}</aside>
    </div>
